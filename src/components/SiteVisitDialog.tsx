...
const inputClass =
  "w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 transition-all focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20";
...
const SiteVisitDialog = ({ open, onOpenChange }: SiteVisitDialogProps) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-[94vw] overflow-hidden border-0 bg-white p-0 shadow-[0_30px_90px_rgba(15,23,42,0.28)] sm:rounded-2xl lg:max-w-5xl">
        <div className="grid lg:grid-cols-[0.95fr_1.35fr]">
          <div className="relative flex min-h-full flex-col overflow-hidden bg-[#10231b] p-5 text-white md:p-6">
            <img src={heroHomeBanner} alt="" className="absolute inset-0 h-full w-full object-cover object-center" loading="lazy" />
            <div className="absolute inset-0 bg-[linear-gradient(135deg,rgba(4,10,8,0.48),rgba(4,10,8,0.68)_58%,rgba(4,10,8,0.78))]" />
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_15%,rgba(247,183,51,0.2),transparent_30%)]" />
            <div className="absolute inset-x-0 bottom-0 h-28 bg-[linear-gradient(0deg,rgba(4,10,8,0.34),transparent)]" />
            <div className="relative z-10 flex min-h-full flex-col">
              <div className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-black/28 backdrop-blur">
                <CalendarCheck className="h-5 w-5 text-[#f7b733]" />
              </div>
              <DialogHeader className="mt-5 text-left">
                <DialogTitle className="text-3xl font-extrabold leading-tight text-white drop-shadow">Book a Free Site Visit</DialogTitle>
                <DialogDescription className="text-base leading-7 text-white/82 drop-shadow">
          ...
          <form onSubmit={handleSubmit} className="space-y-3 p-5 md:p-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                {propertyTypes.map((type) => {
                  ...
                  return (
                    ...
                    className={`flex min-h-14 items-center gap-3 rounded-xl border px-3.5 py-2.5 text-left text-sm font-bold transition-all ${
                        selected
            ...
            <button
              ...
              className="gradient-cta shine flex w-full items-center justify-center gap-2 rounded-xl px-5 py-3.5 text-sm font-extrabold text-slate-950 shadow-lg shadow-secondary/25 transition-all hover:scale-[1.01] disabled:cursor-not-allowed disabled:opacity-70"
            >
